...
const P2PCISPayment = () => {
  ...
  useEffect(() => {
    // Update bank when currency or method changes
  ...
  const updateTransactionStatus = (trackerId: string, newStatus: string) => {
    setTransactions(prev => 
      prev.map(t => 
        t.tracker_id === trackerId 
          ? { ...t, status: newStatus as 'SUCCESS' | 'DECLINED' } 
          : t
      )
    );
  };
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {new Date(transaction.payment_data.payment_expires_at).toLocaleString()}s
                    </p>
                  </div>
